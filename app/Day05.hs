...
parseMapping = do
  ...
  return Mapping{srcStart = srcStart,
...
applyMap m seed = go  (mappings m)
  where go  [] = seed
        go  (mm :mms) =
          if seed >= srcStart mm && seed <= srcEnd mm then
            seed - srcStart mm + destStart mm
          else go  mms
...
applyMapToRange m r = go (mappings m) r
  where go [] r = [r]
        go (m : ms)  r =
          ...
          in if sm <= sr && er <= em  then -- [sm, sr, er, em]
                    --      < Range >
                    --    <  Mapping  >
               [Range { start = sr + shift, end = er + shift}]
             else if sr < sm && sm <= er && er <= em   then -- [sr, sm, er, em]
                    -- < Range >
                    --    < Mapping >
                    (go ms Range {start = sr, end = sm - 1})
                    ++ [Range {start = sm + shift, end = er + shift}]
             else if  sm <= sr && sr <= em && em < er then -- [sm, sr, em, er] 
                    --        < Range >
                    --    < Mapping >
                    [Range {start = sr + shift, end = em + shift}]
                     ++ (go ms Range {start = em + 1, end = er})
             else if  sr < sm && em < er  then
                    -- <    Range    >
                    --    < Mapping >
                    (go ms Range{start = sr, end = sm - 1})
