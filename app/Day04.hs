  ...
  let records = map readRecord
      ...
      go rs total =
        let first = minimum rs
            ...
            newRs =(filter (/= first) rs
                     ++ concat (replicate mult [first + 1
                         .. (min
                              last
                              (first + score))]))
        in if first >= last || null newRs then
             total + mult
           else go  newRs
                   (total + mult)
...
points (Record {name, left, right}) =
        ...
        in if null intersection then 0
